...
class DecBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv3 = dconv1x1(planes * self.expansion, width)
        self.bn3 = norm_layer(planes)

        self.conv2 = dconv3x3(width, width, stride, groups, dilation, output_padding)
        self.bn2 = norm_layer(width)

        self.conv1 = dconv1x1(width, inplanes)
...
class Decoder(nn.Module):
    def __init__(
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, DecBottleneck) and m.bn3.weight is not None:
                    nn.init.constant_(m.bn3.weight, 0)  # type: ignore[arg-type]
                elif isinstance(m, BasicBlock) and m.bn2.weight is not None:
                    nn.init.constant_(m.bn2.weight, 0)  # type: ignore[arg-type]
